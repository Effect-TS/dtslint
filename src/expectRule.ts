...
export class Rule extends Lint.Rules.TypedRule {
  ...
  applyWithProgram(sourceFile: SourceFile, lintProgram: Program): Array<Lint.RuleFailure> {
    const options = this.ruleArguments[0] as Options | undefined
    if (!options) {
      return this.applyWithFunction(
        sourceFile,
        (ctx) => walk(ctx, lintProgram, TsType, "next", /*nextHigherVersion*/ undefined)
      )
    }
    ...
    const getFailures = (
      { path, versionName }: VersionToTest,
      ...
      if (failures.length) {
        console.log(`\x1b[31m[ERROR]   └─ Error(s) found\x1b[0m`)
      }
      return failures
    }

    const maxFailures = getFailures(
      versionsToTest[versionsToTest.length - 1],
      undefined,
      /*writeOutput*/ true
    )
...
function walk(
  ...
  const { duplicates, typeAssertions } = parseAssertions(sourceFile)
  ...
  for (const { actual, expected, node } of unmetExpectations) {
...
function getExpectTypeFailures(
  ...
  ts.forEachChild(sourceFile, function iterate(node) {
    ...
    if (expected !== undefined) {
      ...
      if (
        !expected.split(/\s*\|\|\s*/).some((s) => actual === s || matchReadonlyArray(actual, s))
      ) {
